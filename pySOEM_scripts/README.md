...
```bash
python3 jpvt_client.py reset-position
```

This rebases the controller target after the shaft is moved manually. It does
not rewrite the encoder zero or change the drive's position units.
...
print(send_command({"command": "reset_position"}))
