...
ALLOWED_TRANSITIONS = {
    ...
    MAIN_STATE.INITIALIZED: {
        MAIN_STATE.POSITION_SENSING
    },
    MAIN_STATE.POSITION_SENSING: {
        MAIN_STATE.INITIALIZED,
        MAIN_STATE.SENSOR_READY
    },
    MAIN_STATE.SENSOR_READY: {
        MAIN_STATE.INITIALIZED,
...
class JPVTController:
    ...
    def configure(self) -> None:
        ...
        # ---------------------------------------------------------------
        # RESET FAULT
        statusword = self._get_sdo(SDO.STATUSWORD)
        self._set_sdo(SDO.CONTROLWORD, CW.FAULT_RESET_0)
        if statusword & EPOS_STATE.MASK_FAULT:
            log(f"[EPOS] Resetting startup fault: SW=0x{statusword:04X}")
            self._set_sdo(SDO.CONTROLWORD, CW.FAULT_RESET_1)
            time.sleep(0.2)
            self._set_sdo(SDO.CONTROLWORD, CW.FAULT_RESET_0)
            time.sleep(0.2)
            if self._get_sdo(SDO.STATUSWORD) & EPOS_STATE.MASK_FAULT:
        ...
        # ---------------------------------------------------------------
        # CONFIG JPVT
        self._set_sdo(SDO.OPERATION_MODE, JPVT_MODE)
        time.sleep(0.1)
        if self._get_sdo(SDO.OPERATION_MODE_DISPLAY) != JPVT_MODE:
            raise RuntimeError("[EPOS] JPVT mode was not accepted!")  

        # ---------------------------------------------------------------
        # CONFIG PARAMS
        self._set_sdo(SDO.P_GAIN, self.kp)
        self._set_sdo(SDO.I_GAIN, self.ki)
        self._set_sdo(SDO.D_GAIN, self.kd)
        self._set_sdo(SDO.TARGET_POSITION, self.target_position)
        self._set_sdo(SDO.TARGET_VELOCITY, self.target_velocity)
        self._set_sdo(SDO.TARGET_JOINT_TORQUE, self.target_joint_torque)
        self._set_sdo(SDO.JOINT_TORQUE_LIMIT, self.joint_torque_limit)
        
        actual_limit = self._get_sdo(SDO.JOINT_TORQUE_LIMIT)
        ...
        # ---------------------------------------------------------------
        # CONFIG UNITS
        target_unit = self._get_sdo(SDO.TARGET_POSITION_UNIT)
        fusion_unit = self._get_sdo(SDO.FUSION_POSITION_UNIT)
        ...
        self._set_sdo(SDO.ANTI_ALIAS_CUTOFF, cutoff_hz)
        self._set_sdo(SDO.INTERPOLATION_PERIOD, 2)
        self._set_sdo(SDO.INTERPOLATION_EXPONENT, -3)
        log("[EPOS] Antialiasing and interpolation period set")
        
        # ---------------------------------------------------------------
        # CONFIG PDO RX
        self._set_sdo(SDO.N_MAPPED_RXPDO, 0)
        self._set_sdo(SDO.N_MAPPED_OBJECTS_RXPDO4, 0)
        
        self._set_sdo(SDO.OBJ_1_RXPDO4, RX_MAPPING.CONTROLWORD)
        self._set_sdo(SDO.OBJ_2_RXPDO4, RX_MAPPING.VELOCITY)
        self._set_sdo(SDO.OBJ_3_RXPDO4, RX_MAPPING.POSITION)
        self._set_sdo(SDO.OBJ_4_RXPDO4, RX_MAPPING.TORQUE)
        self._set_sdo(SDO.OBJ_5_RXPDO4, RX_MAPPING.P_GAIN)
        self._set_sdo(SDO.OBJ_6_RXPDO4, RX_MAPPING.I_GAIN)
        self._set_sdo(SDO.OBJ_7_RXPDO4, RX_MAPPING.D_GAIN)
        
        self._set_sdo(SDO.N_MAPPED_OBJECTS_RXPDO4, 7)
        
        rxpdo_object_index = SDO.OBJ_1_RXPDO4.value.index
        self._set_sdo(SDO.OBJ_1_RXPDO, rxpdo_object_index)
        self._set_sdo(SDO.N_MAPPED_RXPDO, 1)
        log("[EPOS] RxPDO set")
        
        # ---------------------------------------------------------------
        # CONFIG PDO TX
        self._set_sdo(SDO.N_MAPPED_TXPDO, 0)
        self._set_sdo(SDO.N_MAPPED_OBJECTS_TXPDO4, 0)
        
        self._set_sdo(SDO.OBJ_1_TXPDO4, TX_MAPPING.STATUSWORD)
        self._set_sdo(SDO.OBJ_2_TXPDO4, TX_MAPPING.FILTERED_VELOCITY)
        self._set_sdo(SDO.OBJ_3_TXPDO4, TX_MAPPING.FILTERED_POSITION)
        self._set_sdo(SDO.OBJ_4_TXPDO4, TX_MAPPING.FILTERED_JOINT_TORQUE)
        
        self._set_sdo(SDO.N_MAPPED_OBJECTS_TXPDO4, 4)
        
        txpdo_object_index = SDO.OBJ_1_TXPDO4.value.index
        self._set_sdo(SDO.OBJ_1_TXPDO, txpdo_object_index)
        self._set_sdo(SDO.N_MAPPED_TXPDO, 1)
        log("[EPOS] TxPDO set")
    ...
    def _set_sdo(self, parameter: SDO, value):
        p = parameter.value
        self.drive.sdo_write(p.index, p.subindex, struct.pack(p.fmt, value))

    def _get_sdo(self, parameter: SDO):
        p = parameter.value
    ...
    def move_state(self, state: MAIN_STATE) -> None:
        ...
        elif state == MAIN_STATE.POSITION_SENSING:
            self.kp = 0
            self.ki = 0
            self.kd = 0
            self.target_joint_torque = 0
        ...
        elif state == MAIN_STATE.SENSOR_READY:
            self.kp = 0
            self.ki = 0
            self.kd = 0
            self.target_joint_torque = 0
    ...
    def q_reached(self, q: int = 0) -> bool:
        # later should be smarter to understand overflows etc.
        _, _, position_raw, _ = self.feedback
        position = position_raw / self.position_scale
        if (position + ZERO_MARGIN > q) and (position - ZERO_MARGIN < q):
            return True
        else:
            return False
...
def handle_command(controller: JPVTController, command):
    ...
    if name == "move":
        controller.request_state(MAIN_STATE.ACTION)
        q, dq, kp, kd = command.get("q"), command.get("dq"), command.get("kp"), command.get("kd")
        controller.move_target(q, dq, kp, kd)
        return {
    ...
    try:
        while not quitting:
            ...
            if not motor_enabled:
                ...
                if state == MAIN_STATE.INITIALIZED:
                    counter = 0
                    controller.move_state(MAIN_STATE.POSITION_SENSING)
                    
                elif state == MAIN_STATE.POSITION_SENSING:
                    counter += 1
                    if counter > N_CYCLES_SENSING_POSITION:
                        controller.move_state(MAIN_STATE.SENSOR_READY)
                        counter = 0
                    ...
                    if controller.q_reached(0):
                        counter += 1
                        if counter > N_CYCLES_HOLD_POSITION:
            ...
            if controller.state != controller.state_requested and controller.state_requested is not None:
                controller.move_state(controller.state_requested)
